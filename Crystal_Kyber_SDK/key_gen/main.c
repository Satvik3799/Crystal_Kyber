...
cortos_printf("Crystal_kyber running\n");
  ...
  cortos_printf ("inv_psis start\n");
    ...
    // point_wise_mult(tcap[0][0], a_0_0, s[0], pwmf);     
    // point_wise_mult(tcap[0][1], a_0_1, s[1], pwmf);
    // point_wise_mult(tcap[1][0], a_1_0, s[0], pwmf);
    // point_wise_mult(tcap[1][1], a_1_1, s[1], pwmf);


    // // Step 5: Compute bcap
    // for ( i = 0; i < 256; i++) {
    //     bcap[0][i] = (tcap[0][0][i] + tcap[0][1][i] + e_0[i]) % q;
    //     bcap[1][i] = (tcap[1][0][i] + tcap[1][1][i] + e_1[i]) % q;
    // }
