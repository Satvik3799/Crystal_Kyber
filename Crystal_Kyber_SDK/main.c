...
void gen_tf( uint16_t *psis, uint16_t *inv_psis) {
    ...
    uint16_t temp_psis[5] = {1, 2, 3, 4, 5};
    uint16_t temp_inv_psis[5] = {1, 2, 3, 4, 5};

    // Copy the values into the arrays passed as arguments
    memcpy(psis, temp_psis, 5 * sizeof(uint16_t));
    memcpy(inv_psis, temp_inv_psis, 5 * sizeof(uint16_t));
};
